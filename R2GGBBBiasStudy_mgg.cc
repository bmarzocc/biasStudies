...
void runfits(int cat=0, int modelNum=-1, int inDirNum=0)
{
  ...
  FILE *fout = fopen("resultsBiasMgg.txt","a");
  ...
  for(int m=0; m<nTruthModels; ++m) {
    ...
    MggBkgTruth[m] = BkgModelFit(w,cat,m); //Ber, Exp, Lan, Lau, Pow
    BkgModelBias(w,cat,MggBkgTruth[m],fout);
  }
...
RooAbsPdf *BkgModelFit(RooWorkspace* w, int c, int modelNum) {
  ...
  for(int i=0; i<totalNDOF; ++i){
    ...
    ctmp->SaveAs(TString::Format("plots/dataBkgMgg_%sN%i_cat%d.png",fitName,i+(modelNum!=0),c));
  ...
  FILE *results = fopen("resultsModelMgg.txt","a");
...
void BkgModelBias(RooWorkspace* w,int c,RooAbsPdf* MggBkgTruth, FILE *fout){
  ...
  for(int k=0; k<totalNDOF; ++k){
    ...
    c0->SaveAs(TString::Format("plots/pullsMgg_gen%s_fitN%d_cat%d.png",MggBkgTruth->GetName(),k+1,c));  
    ...
    c1->SaveAs(TString::Format("plots/toymcMgg_gen%s_fitN%d_cat%d.png",MggBkgTruth->GetName(),k+1,c));
